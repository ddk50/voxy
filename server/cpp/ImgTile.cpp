...
ImgTile::ImgTile(int x, int y, int width, int height, int bits_per_pixel) {
  ...
  this->gen_filename(this->shadow_fname);
...
int ImgTile::disburse() {
  
  if (this->dirty) {
	
	this->gen_filename(this->filename);
	this->openfile(this->filename, &this->fp);
	this->write_png(this->fp);
	fclose(this->fp);
...
void ImgTile::swap() {
  rename(this->filename, this->shadow_fname);
}
...
MosaicMan::MosaicMan(int canvas_width, int canvas_height,
  ...
  this->alloctile(canvas_width, canvas_height, bits_per_pixel, blocklen);
  
  this->sentl = new Sentinel(this->tile,
							 this->xtile_cnt,
							 this->ytile_cnt);
...
void MosaicMan::alloctile(int r_w, int r_h, int bits_per_pixel, int blocklen ) {
  ...
  if ( blocklen <= 0 ) {
	this->tile_len = this->gcd(r_w, r_h);
  } else {
